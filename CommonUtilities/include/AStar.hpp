...
namespace CommonUtilities
{
	...
	)
	{
		...
		const auto getManhattanDistance = [](int aLeftIndex, int aRightIndex)
		{
			const int leftX = aLeftIndex % MapWidth;
			const int leftY = aLeftIndex / MapWidth;
			const int rightX = aRightIndex % MapWidth;
			const int rightY = aRightIndex / MapWidth;
			return std::abs(leftX - rightX) + std::abs(leftY - rightY);
		};

		struct QueueNode
		{
			int myEstimatedTotalDistance = 0;
			int myDistance = 0;
			int myOrder = 0;
			int myIndex = 0;
		};
		...
		openTiles.push({ getManhattanDistance(aStartIndex, anEndIndex), 0, nextQueueOrder++, aStartIndex });
		...
		while (!openTiles.empty())
		{
			...
			const auto tryVisit = [&](int aX, int aY)
			{
				...
				openTiles.push({
					newDistance + getManhattanDistance(neighborIndex, anEndIndex),
					newDistance,
					nextQueueOrder++,
					neighborIndex });
			};
