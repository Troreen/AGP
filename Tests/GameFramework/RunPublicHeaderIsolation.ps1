...
$publicRoot = Join-Path $repository 'Source\GameFramework\Public'
$output = Join-Path $repository "Intermediate\PublicHeaderIsolation\$Configuration"
New-Item -ItemType Directory -Force -Path $output | Out-Null
$headers = @(Get-ChildItem -LiteralPath $publicRoot -Recurse -Filter '*.h' | Sort-Object FullName)
if ($headers.Count -eq 0) { throw 'No public headers found.' }
$items = foreach ($header in $headers) {
    $relative = [IO.Path]::GetRelativePath($publicRoot, $header.FullName).Replace('\','/')
...
$includes = [Security.SecurityElement]::Escape("$publicRoot;$repository\CommonUtilities\include")
$project = @'
...
$leaks = @(Select-String -LiteralPath $log -Pattern 'including file:.*(DirectX|[\\/]D3D[^\\/]*\.h|[\\/]Windows\.h|[\\/]RHI[\\/]|[\\/]GraphicsEngine[\\/]|[\\/]Private[\\/]|[\\/]Integration[\\/])')
if ($leaks.Count) { $leaks; throw 'Public header compilation imported an engine implementation header.' }
Write-Output "PASS: $($headers.Count) public headers independently compile in $Configuration with only Public/shared-math roots and no backend includes. Log: $log"
