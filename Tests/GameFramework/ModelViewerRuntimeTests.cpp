...
public:
    ...
    void RegisterComponents(ComponentRegistry& registry) override { Sample.RegisterComponents(registry); }
    ...
    void OnSceneLoaded(GameContext& context,const SceneId& scene) override
    {
        ...
        auto requireActor=[&](const char* name) {auto* actor=world.FindActor(name);Check(actor!=nullptr,"Actual authored actor missing");return actor;};
        ...
        if(Loads==0) OldCamera=camera->GetRef();
        else {Check(!OldCamera.Get(),"Real-content reload retained old identity");OldCamera=camera->GetRef();}
        ++Loads;Frames=0;
    }
    void OnSceneLoadFailed(GameContext&,const SceneLoadError& error) override
    {
        for(const auto& diagnostic:error.Diagnostics)
            std::cerr<<diagnostic.File<<':'<<diagnostic.Actor<<':'<<diagnostic.Component<<':'<<diagnostic.Property<<' '<<diagnostic.Code<<' '<<diagnostic.Message<<'\n';
    ...
    void Update(GameContext& context,float delta) override
    {
        ...
        const bool rendered=stats.TotalRenderItems>=5 && stats.VisibleRenderItems>0 && stats.BlendedRenderItems>0 && stats.TotalLights==3;
        if(Loads==1 && Frames>=10 && rendered && !ReloadRequested)
        {
            ReloadRequested=true;
            Check(context.GetScenes().Reload(),"Actual sample reload refused");
        }
        if(Loads==2 && Frames>=20 && rendered && !Rendered) {Rendered=true;Check(context.GetScenes().Load(SceneId{"Empty"}),"Empty replacement request refused");}
        if(Loads==3 && Frames>=10 && stats.TotalRenderItems==0 && stats.TotalLights==0) context.RequestQuit();
    }
    void Shutdown(GameContext& context) override {++Shutdowns;Sample.Shutdown(context);}
};
class SampleSource final : public GameFrameworkIntegration::ISceneSource
{
    ...
    {
        if(id.Value=="Empty") return {SceneData{}, {}};
...
int main(int argc,char** argv)
{
    ...
    try
    {
        ...
        config.ShowWindow=false;config.Width=640;config.Height=360;
        ...
        for(const auto& error:diagnostics.Errors) std::cerr<<error<<'\n';
        ...
        std::cout<<"PASS: actual ModelViewer registration, assets, authored source, callbacks, reload and hidden rendering ("<<(config.ThreadedUpdate?"threaded":"sync")<<")\n";
        return 0;
    }
