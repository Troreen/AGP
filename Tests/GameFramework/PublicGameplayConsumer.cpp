...
template <class T>
concept CanTick = requires(T& value) { value.Update(0.01f); };
template <class T>
concept CanActivate = requires(T& value) { value.Activate(); };
template <class T>
concept CanShutdown = requires(T& value) { value.Shutdown(); };
template <class T>
concept CanPrepare = requires { &T::Prepare; };
template <class T>
concept CanFlush = requires { &T::Flush; };
template <class T>
concept CanQueue = requires { &T::QueueStructure; };
template <class T>
concept CanFreeze = requires(T& value) { value.Freeze(); };
template <class T>
concept CanSeeState = requires(T& value) { value.GetState(); };
template <class T>
concept CanSeeOwnedActors = requires(T& value) { value.GetActors(); };
template <class T>
concept CanSeeOwnedComponents = requires(T& value) { value.GetComponents(); };
template <class T>
concept CanClearInput = requires(T& value) { value.ClearPressed(); };
template <class T>
concept CanMergeInput = requires(T& value) { value.Merge(value); };
...
static_assert(!std::is_copy_constructible_v<World> && !std::is_default_constructible_v<World> &&
              !std::is_constructible_v<World, const GameInput*>);
...
template <class T>
concept CanSetRawParent = requires(T& value) { value.SetParent(&value); };
...
template <class T>
concept CanBorrowBackendCamera = requires(T& value) { value.GetCamera(); };
template <class T>
concept CanBorrowBackendJoints = requires(T& value) { value.GetJointTransforms(); };
template <class T>
concept CanSeeMaterialList = requires(T& value) { value.GetMaterialList(); };
template <class T>
concept CanSetActorPosition = requires(T& value) { value.SetPosition({0, 0, 0}); };
...
class NeedsOffset final : public Component
{
	...
	{
		myOffset = references.Require<SceneComponent>("Offset");
	}
