...
void TestCulling()
{
	...
	const std::array<CU::Vector3f, 6> touching = {
	    {{-1.1f, 0, 0.5f}, {1.1f, 0, 0.5f}, {0, -1.1f, 0.5f}, {0, 1.1f, 0.5f}, {0, 0, -0.1f}, {0, 0, 1.1f}}};
...
void TestRouting()
{
	const auto mixed = RenderItemRouting::Classify(std::array{true, false, true}, [](bool opaque)
	{
		return opaque;
	});
	Check(mixed.Opaque && mixed.Blended, "mixed mesh must enter both passes");
	const auto empty = RenderItemRouting::Classify(std::array<bool, 0>{}, [](bool v)
	{
		return v;
	});
	Check(!empty.Opaque && !empty.Blended, "empty mesh classified");
	const auto opaqueOnly = RenderItemRouting::Classify(std::array{true, true}, [](bool v)
	{
		return v;
	});
	...
	RenderItemRouting::Sort(opaque, blended, [&distances](size_t i)
	{
		return distances[i];
	});
...
void TestQueue()
{
	...
	auto* cancelled = queue.BeginBuild();
	queue.CancelBuild(cancelled);
	queue.Publish(cancelled);
	Check(queue.AcquireLatest() == nullptr, "cancelled snapshot published");
	auto* first = queue.BeginBuild();
	first->Sequence = 1;
	queue.Publish(first);
	...
	while (!done)
	{
		...
		{
			for (int value : next->Values)
			{
				Check(value == next->Sequence, "torn snapshot");
			}
		}
	}
	producer.join();
...
void TestGameLoop()
{
	...
	std::string order;
	auto fixed = [&fixedCount, &order](float dt, const GameInput& sample)
	{
	...
	auto update = [&updateCount, &order](float, const GameInput& sample)
	{
		Check(sample.IsKeyPressed(Keys::P) == (updateCount == 0), "variable edge lost or repeated");
		++updateCount;
		order += 'U';
	};
	auto late = [&lateCount, &order](float, const GameInput&)
	{
		++lateCount;
		order += 'L';
	};
	...
	int steps = 0;
	bounded.Advance(10.0f, {},
	                [&steps](float, const GameInput&)
	{
		++steps;
	},
	                [](float dt, const GameInput&)
	{
	...
	try
	{
		...
		}, [&lateAfterFailure](float, const GameInput&)
		{
			lateAfterFailure = true;
		});
	}
	catch (const std::runtime_error&)
	{
