                                           ...
                                           UINT* pBytes)
{
	...
	else
	{
		...
		if (pParentData)
		{
			...
			{
				candidates.emplace_back(it->second.Path.parent_path() / pFileName);
			}
		}
		candidates.emplace_back(programPath / pFileName);
		candidates.emplace_back(myShaderRoot / pFileName);

		// Avoid weakly_canonical here. On Windows it can throw for a missing
		// first candidate before the fallback include directory is considered.
		sourcePath = candidates.front().lexically_normal();
		for (const auto& candidate : candidates)
		{
			std::error_code error;
			if (std::filesystem::exists(candidate, error) && !error)
			{
				sourcePath = candidate.lexically_normal();
				break;
			}
		}
	}
