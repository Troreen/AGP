...
void GameApplication::Impl::BuildAndPublishRenderSnapshot()
{
	...
	try
	{
        const bool hasCamera = GameFrameworkInternal::WorldRenderBridge::Build(*myContext.myState->myWorld, GraphicsEngine::Get(), *snapshot);
        if (!hasCamera && !myMissingCameraReported) GFLOG(Warning, "No active camera; presenting an empty frame");
...
bool GameApplication::Impl::ReplaceScene()
{
    ...
    try
    {
        ...
        WorldAccess::Construct([&] { source = session.mySource->Load(requested, context); }, "Scene source");
