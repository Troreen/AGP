                                                     ...
                                                     GraphicsEngine::RenderSceneSnapshot& snapshot)
{
	...
	if (!camera || !camera->HasBegunPlay() || !camera->IsEnabled() || !camera->GetOwner()->IsActiveInHierarchy())
	{
		return false;
	}
	...
	for (const auto& actor : actors)
	{
		...
		lights.clear();
		actor->GetComponentsOfType(lights);
		for (const auto* light : lights)
		{
			if (light && light->HasBegunPlay() && light->IsEnabled())
			{
				snapshot.RelevantLights.push_back(CopyLight(*light));
			}
		}
		meshes.clear();
		actor->GetComponentsOfType(meshes);
		for (const auto* component : meshes)
		{
			if (!component || !component->HasBegunPlay() || !component->IsEnabled() || !component->IsVisible())
			{
				continue;
			}
			...
			if (const auto* joints = RenderAccess::JointTransforms(*component))
			{
