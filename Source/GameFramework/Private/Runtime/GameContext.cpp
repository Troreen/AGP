...
{
	GameFrameworkInternal::WorldAccess::Shutdown(*myState->myWorld);
}

World& GameContext::GetWorld()
{
	return *myState->myWorld;
}

const GameInput& GameContext::GetInput() const
{
	return myState->myInput;
}

SceneService& GameContext::GetScenes()
{
	return myState->myScenes;
}

const AssetLookup& GameContext::GetAssets() const
{
	return *myState->myAssets;
}

const GameTime& GameContext::GetTime() const
{
	return myState->myTime;
}

const std::filesystem::path& GameContext::GetContentRoot() const
{
	return myState->myContentRoot;
}

CommonUtilities::Vector2u GameContext::GetClientSize() const
{
	return myState->myClientSize;
}
