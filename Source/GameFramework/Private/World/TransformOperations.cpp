...
{
	static DirectX::XMMATRIX ToDirectX(const CommonUtilities::Matrix4f& matrix)
	{
		...
		for (int r = 0; r < 4; ++r)
		{
			for (int c = 0; c < 4; ++c)
			{
				data.m[r][c] = matrix(r + 1, c + 1);
			}
		}
		return DirectX::XMLoadFloat4x4(&data);
	}

	static CommonUtilities::Matrix4f FromDirectX(DirectX::FXMMATRIX matrix)
	{
		DirectX::XMFLOAT4X4 data;
	...
	bool SetLocalMatrix(CommonUtilities::Transform& target, const CommonUtilities::Matrix4f& matrix)
	{
		...
		if (!DirectX::XMMatrixDecompose(&scale, &rotation, &position, ToDirectX(matrix)))
		{
			return false;
		}
		DirectX::XMFLOAT3 s, p;
		DirectX::XMFLOAT4 q;
		...
		for (int r = 1; r <= 4; ++r)
		{
			for (int c = 1; c <= 4; ++c)
			{
				if (!std::isfinite(rebuilt(r, c)) || std::abs(rebuilt(r, c) - matrix(r, c)) > 0.0001f * (1.f + std::abs(matrix(r, c))))
				{
					return false;
				}
			}
		}
		target.SetPosition(candidate.GetPosition());
		target.SetRotation(candidate.GetRotation());
		target.SetScale(candidate.GetScale());
		return true;
	}
	...
	bool ChangeParent(CommonUtilities::Transform& local, CommonUtilities::Transform* parent, ReparentMode mode)
	{
		if (mode == ReparentMode::KeepWorld)
		{
			...
			if (!SetLocalMatrix(local, FromDirectX(world)))
			{
				return false;
			}
		}
