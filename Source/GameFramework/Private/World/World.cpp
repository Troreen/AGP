...
void GameFrameworkInternal::WorldAccess::Construct(const std::function<void()>& callback, const char* phase)
{
    ...
    const auto before = rejectedMutations;
    callback();
    if (rejectedMutations != before) throw std::logic_error(std::string(phase) + " attempted forbidden gameplay mutation");
}
...
GameFrameworkInternal::ObjectIdentity World::Allocate(Actor* actor, Component* component)
{
    ...
    GameFrameworkInternal::ObjectIdentity handle; handle.mySlots = mySlots; handle.myIndex = index; handle.myGeneration = slot.generation;
    return handle;
}
