...
#include <stdexcept>
#include <limits>
#include "Runtime/Internal/ObjectSlots.h"
...
void World::EnsureMutationAllowed() const
{
	...
	{
		++rejectedMutations;
		throw std::logic_error("Structural or session mutation is forbidden during scene configuration/resolution");
	}
}
...
void World::EnsureComponentMutationAllowed(const Component* component) const
{
	...
	{
		++rejectedMutations;
		throw std::logic_error("Only the reader target can be configured; resolution is read-only");
	}
}
...
void GameFrameworkInternal::WorldAccess::Configure(Component& component, const std::function<void()>& callback)
{
	...
	public:
		...
		explicit Guard(const Component& target)
		{
			configuringComponent = &target;
		}

		~Guard()
		{
			configuringComponent = Previous;
		}
	} guard(component);

	const auto before = rejectedMutations;
	callback();
	if (rejectedMutations != before)
	{
		throw std::logic_error("Forbidden mutation attempted by scene reader");
	}
}

void GameFrameworkInternal::WorldAccess::Construct(const std::function<void()>& callback, const char* phase)
{
	class Guard
	{
	public:
	...
	if (rejectedMutations != before)
	{
		throw std::logic_error(std::string(phase) + " attempted forbidden gameplay mutation");
	}
}
...
SceneService& World::GetScenes() const
{
	if (!myScenes)
	{
		throw std::logic_error("World has no scene service");
	}
	return *myScenes;
}

const AssetLookup& World::GetAssets() const
{
	static const AssetLookup empty;
	return myAssets ? *myAssets : empty;
}

const GameTime& World::GetTime() const
{
	static const GameTime empty;
	return myTime ? *myTime : empty;
}

Actor* GameFrameworkInternal::ObjectIdentity::ResolveActor() const
{
	auto slots = mySlots.lock();
	if (!slots || myIndex >= slots->slots.size())
	{
		return nullptr;
	}
...
Component* GameFrameworkInternal::ObjectIdentity::ResolveComponent() const
{
	auto slots = mySlots.lock();
	if (!slots || myIndex >= slots->slots.size())
	{
...
GameFrameworkInternal::ObjectIdentity World::Allocate(Actor* actor, Component* component)
{
	size_t index = 0;
	while (index < mySlots->slots.size() &&
	       (mySlots->slots[index].actor || mySlots->slots[index].component || mySlots->slots[index].generation == 0))
	{
		++index;
	}
	if (index == mySlots->slots.size())
	{
		mySlots->slots.emplace_back();
	}
	auto& slot = mySlots->slots[index];
	slot.actor = actor;
	slot.component = component;
	GameFrameworkInternal::ObjectIdentity handle;
	handle.mySlots = mySlots;
	handle.myIndex = index;
	handle.myGeneration = slot.generation;
	return handle;
}

void World::Invalidate(const GameFrameworkInternal::ObjectIdentity& handle)
{
	auto& slot = mySlots->slots[handle.myIndex];
	slot.actor = nullptr;
	slot.component = nullptr;
	slot.generation = slot.generation == std::numeric_limits<uint64_t>::max() ? 0 : slot.generation + 1;
}
...
Actor* World::SpawnActor(std::string name)
{
	...
	auto* raw = actor.get();
	raw->SetWorld(this);
	raw->myHandle = Allocate(raw, nullptr);
...
std::vector<Actor*> World::FindActors(const std::string& name) const
{
	...
	for (const auto* list : {&myActors, &myPendingActors})
	{
		...
		{
			if (!actor->myPendingDestroy && actor->GetName() == name)
			{
...
Actor* World::FindActor(const std::string& name) const
{
	...
	if (matches.size() > 1)
	{
		GFLOG(Warning, "Ambiguous actor display name: {}", name);
	}
...
{
	if (auto* spatial = dynamic_cast<SceneComponent*>(component.get()))
	{
		spatial->myTransform.myWorld = this;
		spatial->myTransform.myComponent = component.get();
		spatial->myTransform.myValue.SetParent(&actor.myTransform.myValue);
	}
...
{
	EnsureMutationAllowed();
	if (actor.GetWorld() != this || actor.myPendingDestroy)
	{
		return;
	}
...
{
	EnsureMutationAllowed();
	if (&component.GetWorld() != this || component.myPendingDestroy)
	{
		return;
	}
	if (auto* spatial = dynamic_cast<SceneComponent*>(&component))
	{
		for (auto* list : {&component.GetOwner()->myComponents, &component.GetOwner()->myPendingComponents})
		{
			for (auto& child : *list)
			{
				if (auto* s = dynamic_cast<SceneComponent*>(child.get()); s && s->GetParent() == spatial)
				{
					DestroyComponent(*s);
				}
			}
...
bool World::ConnectBatch(const std::vector<Component*>& batch, SceneDiagnostics& diagnostics)
{
	...
	for (auto* c : batch)
	{
		...
		{
			context.Error({}, "Unknown exception in ResolveReferences");
		}
...
{
	if (myState != State::Constructing)
	{
		throw std::logic_error("Prepare requires a constructing world");
	}
	myInBoundary = true;
	std::vector<Component*> batch;
	for (auto& a : myActors)
	{
		for (auto& c : a->myComponents)
		{
			batch.push_back(c.get());
		}
	}
...
void World::Activate()
{
	...
	{
		throw std::logic_error("Activate requires a prepared world");
	}
...
bool World::Flush(SceneDiagnostics& diagnostics)
{
	...
	auto actors = std::move(myPendingActors);
	myPendingActors.clear();
	std::vector<std::pair<Actor*, std::vector<std::unique_ptr<Component>>>> additions;
	for (auto& a : myActors)
	{
		additions.emplace_back(a.get(), std::move(a->myPendingComponents));
		a->myPendingComponents.clear();
	}
	for (auto& a : actors)
	{
		additions.emplace_back(a.get(), std::move(a->myPendingComponents));
	...
	for (auto& a : actors)
	{
		addedActors.push_back(a.get());
		myActors.push_back(std::move(a));
	}
	std::vector<Component*> batch;
	for (auto& [a, components] : additions)
	{
		for (auto& c : components)
		{
			batch.push_back(c.get());
			a->myComponents.push_back(std::move(c));
		}
	}
...
void World::CollectDestroyed() noexcept
{
	...
	for (auto* list : {&myActors, &myPendingActors})
	{
		for (auto& actor : *list)
		{
			for (auto* components : {&actor->myComponents, &actor->myPendingComponents})
			{
				for (auto& c : *components)
				{
					if (c->myPendingDestroy)
					{
						removedComponents.push_back(std::move(c));
					}
				}
				std::erase_if(*components, [](const auto& c)
				{
					return !c;
				});
			}
		...
		for (auto& a : *list)
		{
			if (a->myPendingDestroy)
			{
				removedActors.push_back(std::move(a));
			}
		}
		std::erase_if(*list, [](const auto& a)
		{
			return !a;
		});
	}
	auto removed = [&removedComponents](Component* c)
	{
		return std::any_of(removedComponents.begin(), removedComponents.end(), [c](const auto& owned)
		{
			return owned.get() == c;
		});
	};
	std::vector<Component*> endOrder;
	for (auto it = myActivationOrder.rbegin(); it != myActivationOrder.rend(); ++it)
	{
		if (removed(*it))
		{
			endOrder.push_back(*it);
		}
	}
	...
	auto depth = [](Component* c)
	{
		...
		if (auto* spatial = dynamic_cast<SceneComponent*>(c))
		{
			t = &spatial->myTransform.myValue;
		}
		while (t->GetParent())
		{
			++n;
	...
	std::stable_sort(endOrder.begin(), endOrder.end(), [depth](auto* a, auto* b)
	{
	...
	for (auto* c : endOrder)
	{
		...
		{
			GFLOG(Error, "EndPlay failed during cleanup");
		}
	}
	std::stable_sort(removedComponents.begin(), removedComponents.end(), [depth](const auto& a, const auto& b)
	{
		return depth(a.get()) > depth(b.get());
	});
	...
	for (auto& c : removedComponents)
	{
		if (auto* spatial = dynamic_cast<SceneComponent*>(c.get()))
		{
			spatial->myTransform.myValue.SetParent(nullptr);
		}
	}
	for (auto& a : removedActors)
	{
		a->myTransform.myValue.SetParent(nullptr);
		Invalidate(a->myHandle);
	}
...
{
	EnsureMutationAllowed();
	if (camera && (&camera->GetWorld() != this || camera->IsPendingDestroy()))
	{
		return false;
	}
	myCamera = camera ? camera->GetRef<CameraComponent>() : ComponentRef<CameraComponent>{};
	return true;
}
