...
namespace
{
	...
	bool Normalize(const CommonUtilities::Quaternion<float>& value, CommonUtilities::Quaternion<float>& result)
	{
		...
		const double length =
		    std::sqrt(double(value.w) * value.w + double(value.x) * value.x + double(value.y) * value.y + double(value.z) * value.z);
		if (length == 0)
		{
			return false;
		}
...
void Transform::EnsureMutationAllowed() const
{
	if (myWorld)
	{
		myWorld->EnsureComponentMutationAllowed(myComponent);
	}
}
...
bool Transform::SetLocalPose(const LocalPose& pose)
{
	...
	if (!Finite(pose.Position) || !Finite(pose.Scale) || !Normalize(pose.Rotation, rotation))
	{
		return false;
	}
...
{
	return SetLocalRotationRadians(CommonUtilities::Maths::DegreesToRadians(yaw), CommonUtilities::Maths::DegreesToRadians(pitch),
	                               CommonUtilities::Maths::DegreesToRadians(roll));
}
...
bool Transform::SetWorldPosition(const CommonUtilities::Vector3f& position)
{
	...
	matrix(4, 1) = position.x;
	matrix(4, 2) = position.y;
	matrix(4, 3) = position.z;
