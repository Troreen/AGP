...
Actor::Actor(std::string aName) : myName(std::move(aName))
{
}
...
void Actor::RemoveAllComponents()
{
	...
	for (auto& c : myPendingComponents)
	{
		c->Destroy();
	}
}
...
std::string Actor::NextComponentName() const
{
	...
	do
	{
		name = "Component " + std::to_string(++number);
	} while (FindComponent(name));
	return name;
}
...
{
	if (!CanAttach() || (parent && (parent->GetWorld() != myWorld || parent->myPendingDestroy)))
	{
		return false;
	}
	if (parent && myAdmitted && !parent->myAdmitted)
	{
		return false;
	}
	...
	if (!GameFrameworkInternal::ChangeParent(myTransform.myValue, parent ? &parent->myTransform.myValue : nullptr, mode))
	{
		return false;
	}
