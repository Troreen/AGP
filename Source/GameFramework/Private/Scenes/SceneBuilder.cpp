...
SceneBuildResult SceneBuilder::Build(const SceneData& scene, const ComponentRegistry& registry, const SceneBuildServices& services)
{
    ...
    try
    {
        ...
        for (const auto& actor : scene.Actors)
        {
            ...
            runtimeActor->GetTransform().SetLocalPose(actor.Pose); runtimeActor->SetActive(actor.Active);
            ...
            for (const auto& component : actor.Components)
            {
                ...
                if (spatial)
                {
                    if (component.Pose) spatial->GetTransform().SetLocalPose(*component.Pose);
