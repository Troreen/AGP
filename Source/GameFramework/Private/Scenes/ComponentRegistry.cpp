...
Component* ComponentRegistry::Create(const std::string& type, Actor& actor, const std::string& name) const
{
    ...
    if (it == myTypes.end()) throw std::invalid_argument("Unknown component type: " + type);
    std::unique_ptr<Component> component;
    GameFrameworkInternal::WorldAccess::Construct([&] { component = it->second.Factory(); });
    if (!component) throw std::runtime_error("Component factory returned no component");
