...
void GameFrameworkInternal::RegisterBuiltInComponents(ComponentRegistry& registry)
{
	...
	registry.Register<SkeletalMeshComponent>("agp.SkeletalMesh", [](SkeletalMeshComponent& component, SceneReader& fields)
	{
		...
		if (!mesh->HasSkeleton())
		{
			fields.Error("mesh", "Skeletal mesh requires a valid skeleton");
		}
		...
		if (fields.Has("animation"))
		{
			...
			if (!clip || !clip->IsValid() || !std::isfinite(clip->FramesPerSecond) || !std::isfinite(clip->Duration) || clip->Duration < 0)
			{
				fields.Error("animation", "Animation is unavailable or invalid");
			}
			else if (!component.PlayAnimation(animation, loop))
			{
				fields.Error("animation", "Animation could not start");
			}
		}
	});
	registry.Register<DirectionalLightComponent>("agp.DirectionalLight", [](DirectionalLightComponent& component, SceneReader& fields)
	{
		ReadLight(component, fields);
	});
	registry.Register<PointLightComponent>("agp.PointLight", [](PointLightComponent& component, SceneReader& fields)
	{
		ReadLight(component, fields);
	});
	registry.Register<SpotLightComponent>("agp.SpotLight", [](SpotLightComponent& component, SceneReader& fields)
	{
		ReadLight(component, fields);
	});
}
