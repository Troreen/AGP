...
{
	if (myState->BoundWorld)
	{
		GameFrameworkInternal::WorldAccess::EnsureMutationAllowed(*myState->BoundWorld);
	}
	if (id.Value.empty())
	{
		throw std::invalid_argument("SceneId must not be empty");
	}
	std::scoped_lock lock(myState->Mutex);
	if (!myState->Accepting)
	{
		return false;
	}
...
SceneLoadStatus SceneService::GetStatus() const
{
	std::scoped_lock lock(myState->Mutex);
	return myState->Status;
}

std::optional<SceneId> SceneService::GetCurrent() const
{
	std::scoped_lock lock(myState->Mutex);
	return myState->Current;
}

std::optional<SceneLoadError> SceneService::GetLastError() const
{
	std::scoped_lock lock(myState->Mutex);
