...
SceneReader::SceneReader(const ComponentRecord& record, SceneDiagnostic source, SceneDiagnostics& diagnostics, const AssetLookup* assets,
                         CommonUtilities::Vector2u clientSize, std::function<Actor*(const std::string&)> actorLookup,
                         std::function<Component*(const ObjectAddress&)> componentLookup)
    : myImpl(std::make_unique<Impl>(
          Impl{record, std::move(source), diagnostics, assets, clientSize, std::move(actorLookup), std::move(componentLookup), {}, {}}))
{
}

SceneReader::~SceneReader() = default;

bool SceneReader::Has(const std::string& field) const
{
	return myImpl->Record.Properties.contains(field);
}
...
float SceneReader::OptionalFloat(const std::string& field, float fallback)
{
	...
	{
		myImpl->Error(field, "Expected a numeric property", "wrong-type");
		return fallback;
	}
	if (!std::isfinite(number) || std::abs(number) > std::numeric_limits<float>::max())
	{
		myImpl->Error(field, "Number must be finite and fit a float", "invalid-value");
		return fallback;
	}
	return static_cast<float>(number);
}

bool SceneReader::OptionalBool(const std::string& field, bool fallback)
{
	return myImpl->Optional(field, fallback);
}

std::string SceneReader::OptionalString(const std::string& field, std::string fallback)
{
...
CommonUtilities::Vector3f SceneReader::OptionalVector3(const std::string& field, CommonUtilities::Vector3f fallback)
{
	...
	{
		myImpl->Error(field, "Vector must be finite", "invalid-value");
		return fallback;
	}
	return value;
}
...
CommonUtilities::Quaternion<float> SceneReader::OptionalQuaternion(const std::string& field, CommonUtilities::Quaternion<float> fallback)
{
	...
	{
		myImpl->Error(field, "Quaternion must be finite and nonzero", "invalid-value");
		return fallback;
	}
	return value;
}
...
CommonUtilities::Vector4f SceneReader::OptionalColor(const std::string& field, CommonUtilities::Vector4f fallback)
{
	...
	{
		myImpl->Error(field, "Color must be finite", "invalid-value");
		return fallback;
	}
	return value;
}

AssetId SceneReader::OptionalAsset(const std::string& field, AssetId fallback)
{
	return myImpl->Optional(field, std::move(fallback));
}
...
AssetId SceneReader::RequiredAsset(const std::string& field)
{
	if (!Has(field))
	{
		myImpl->Error(field, "Required asset property is missing", "missing-property");
		return {};
	}
	auto result = OptionalAsset(field);
	if (result.Value.empty())
	{
		myImpl->Error(field, "Asset identifier cannot be empty", "invalid-value");
	}
	return result;
}

void SceneReader::Error(std::string field, std::string message)
{
	myImpl->Error(field, std::move(message), "invalid-value");
}

const AssetLookup& SceneReader::GetAssets() const
{
	class EmptyLookup final : public AssetLookup
	{
	public:
		MeshAsset FindMesh(const AssetId&) const override
		{
			return {};
		}

		MaterialAsset FindMaterial(const AssetId&) const override
		{
			return {};
		}
	};
...
CommonUtilities::Vector2u SceneReader::GetClientSize() const
{
	return myImpl->ClientSize;
}
                       ...
                       std::function<bool(Actor*, Component*)> assign)
{
	...
	if (!value)
	{
		if (requirement == ReferenceRequirement::Required)
		{
			myImpl->Error(field, "Required reference property is missing", "missing-property");
		}
		return;
	}
	...
	if (!address)
	{
		myImpl->Error(field, "Expected an object address", "wrong-type");
		return;
	}
	if (address->ActorId.empty() || (component ? address->ComponentId.empty() : !address->ComponentId.empty()))
	{
		myImpl->Error(field, "Object address does not match the reference kind", "invalid-reference");
		return;
	}
...
void SceneReader::Finish()
{
	...
	{
		if (!myImpl->Consumed.contains(field))
		{
			myImpl->Error(field, "Unknown or unconsumed property", "unknown-property");
		}
	}
}
