...
bool SceneComponent::SetParent(SceneComponent* parent, ReparentMode mode)
{
    ...
    if (!GetWorld().AcceptsChanges()) return false;
    if (IsPendingDestroy() || (parent && (parent->GetOwner() != GetOwner() || parent->IsPendingDestroy()))) return false;
    if (myAdmitted && parent && !parent->myAdmitted) return false;
    ...
    if (!GameFrameworkInternal::ChangeParent(myTransform.myValue, parentValue, mode)) return false;
