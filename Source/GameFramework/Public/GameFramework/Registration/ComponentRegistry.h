...
public:
	...
	{
		RegisterFactory<T>(std::move(type), []
		{
	...
	template <class T>
	void RegisterFactory(std::string type, std::function<std::unique_ptr<T>()> factory, std::function<void(T&, SceneReader&)> reader = {})
	{
		...
		if (type.empty() || myTypes.contains(type))
		{
			throw std::invalid_argument("Duplicate or empty component type: " + type);
		}
		if (!factory)
		{
			throw std::invalid_argument("A component factory is required");
		}
		Entry entry;
		entry.Factory = [factory = std::move(factory)]() -> std::unique_ptr<Component>
		{
			return factory();
		};
		entry.Reader = [reader = std::move(reader)](Component& component, SceneReader& fields)
		{
			if (reader)
			{
				reader(dynamic_cast<T&>(component), fields);
			}
		};
...
private:
	...
	friend class GameFrameworkInternal::RegistryAccess;
