...
#include "GameFramework/Runtime/Internal/AssetAccess.h"
...
namespace
{
    bool ReadMesh(MeshComponentBase& component, SceneReader& fields)
    {
        ...
        const auto mesh = GameFrameworkInternal::AssetAccess::Mesh(fields.GetAssets().FindMesh(meshId));
        ...
        std::vector<std::shared_ptr<MaterialInterface>> bindings;
        bool valid = true;
        for (size_t index = 0; index < materials.size(); ++index)
        {
            auto material = GameFrameworkInternal::AssetAccess::Material(fields.GetAssets().FindMaterial(materials[index]));
            if (!material)
...
void GameFrameworkInternal::RegisterBuiltInComponents(ComponentRegistry& registry)
{
    ...
    registry.Register<SkeletalMeshComponent>("agp.SkeletalMesh", [](SkeletalMeshComponent& component, SceneReader& fields)
    {
        ...
        if (!meshReady) return;
        if (!component.GetMesh()->HasSkeleton()) fields.Error("mesh", "Skeletal mesh requires a valid skeleton");
        ...
        {
            const auto clip = animation.empty() ? nullptr : component.GetMesh()->GetAnimation(animation);
