...
namespace GameFrameworkIntegration
{
	...
	public:
		...
		{
			MeshAsset value;
			value.myResource = std::move(mesh);
			myMeshes.insert_or_assign(id.Value, std::move(value));
		}

		void BindMaterial(const AssetId& id, std::shared_ptr<::MaterialInterface> material)
		{
			MaterialAsset value;
			value.myResource = std::move(material);
			myMaterials.insert_or_assign(id.Value, std::move(value));
		}

		MeshAsset FindMesh(const AssetId& id) const override
		{
			auto i = myMeshes.find(id.Value);
			return i == myMeshes.end() ? MeshAsset{} : i->second;
		}

		MaterialAsset FindMaterial(const AssetId& id) const override
		{
			auto i = myMaterials.find(id.Value);
			return i == myMaterials.end() ? MaterialAsset{} : i->second;
		}
