...
bool MeshLibrary::LoadFBXMesh(const std::filesystem::path& aPath)
{
	...
	{
		GAMELOG(Warning, "Could not load FBX mesh '{}': file was not found.", aPath.string());
		return false;
	}
	...
	{
		GAMELOG(Warning, "Could not load FBX mesh '{}': imported mesh was empty.", resolvedPath.string());
		return false;
	}
	...
	if (elements.empty())
	{
		GAMELOG(Warning, "Could not load FBX mesh '{}': no renderable mesh elements were found.", resolvedPath.string());
		return false;
	}
	...
	GAMELOG(Log, "Loaded FBX mesh '{}' with {} elements.", meshName, importedMesh.Elements.size());
	return true;
}
