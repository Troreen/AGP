...
void ModelViewer::Initialize(GameContext& context)
{
	...
	MVLOG(
	    Log,
	    "Game ready: RMB + WASD/Space/Ctrl camera, R pause chest spin, numpad 0-3 animation, 7-9 lights, Shift+7-9 place lights, F5 reload, F7 spawn/destroy hierarchy, Esc quit");
}
...
void ModelViewer::Update(GameContext& context, float)
{
	...
	{
		context.GetScenes().Reload();
	}
	if (context.GetInput().IsKeyPressed(Keys::F7))
	{
		auto& world = context.GetWorld();
		if (auto* demo = world.FindActor("Hierarchy Demo"))
		{
			demo->Destroy();
		}
		else if (auto* source = world.FindActor("SM_Chest Actor"))
		{
			...
			auto* demo = world.SpawnActor("Hierarchy Demo");
			...
			pivot->GetTransform().SetLocalPosition({0, 50, 0});
			for (int i = 0; original && i < 2; ++i)
			{
				auto* mesh = demo->AddComponent<StaticMeshComponent>("Chest " + std::to_string(i), original->GetMesh());
				mesh->GetTransform().SetLocalPosition({i == 0 ? -60.f : 60.f, 0, 0});
				mesh->GetTransform().SetLocalScale({.5f, .5f, .5f});
