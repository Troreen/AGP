...
void ModelViewer::Update(GameContext& context, float)
{
    ...
    if (context.GetInput().IsKeyPressed(Keys::F7))
    {
        ...
        else if (auto* source = world.FindActor("SM_Chest Actor"))
        {
            ...
            demo->GetTransform().SetLocalPosition({350,0,285}); demo->AddComponent<SpinComponent>("Spin");
            ...
            for (int i = 0; original && i < 2; ++i)
            {
                ...
                for (unsigned slot = 0; slot < original->GetMaterialCount(); ++slot) mesh->SetMaterial(slot,original->GetMaterial(slot));
            }
        }
    }
}
