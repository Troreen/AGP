...
The follow-up began from clean `game-framework` at `b9ed4d7`. The user requested explicit braces, expanded function/lambda bodies, data-only structs and explicit captures throughout the repository. The existing vendor/generated exclusions were retained; generated ModelViewer resource.h was added to them. No applicable AGENTS.md was found. No public gameplay semantics were changed.

Authored Source and Tests C++ now follows the tightened clang-format configuration. Function-bearing types, including implementation helpers, equality-bearing IDs, logger macros and material traits, are classes with explicit access. Public aggregate initialization remains supported. GameApplication::Impl exposes only construction/destruction and Run; helpers and state are private, and its method definitions are separate from the declaration. Lambda dependencies are named, with asynchronous loop indices still copied and referenced work data retained until workers join. Six authored HLSL files now brace previously compact control flow.

Independent review of production, tests, shaders and guidance found no unintended expression, lifetime, access, macro or control-flow changes. Test assertions, failure paths and calls were preserved. Exact formatter output matches all 127 authored C++ source files and all eight test files; source/test capture scans and diff whitespace checks pass.

Independent execution passed all 12 Debug/Release project builds, all 25 isolated public headers in both configurations, four CPU-suite runs, two extraction fixtures, two immediate logger-drain/exit runs, 84 host cases and four actual ModelViewer runtime cases. All six edited source/runtime shader hashes match. D3D debug queues were available and contained no ERROR/CORRUPTION messages in the 60 normal-result host cases and all four sample runs; 24 expected-exception cases validate cleanup separately. Final failures: zero. Existing C4100/C4324/C4456/C4702/LNK4099 warnings were corroborated against the earlier M4 logs; no new warning categories appeared.

Generated logs use `Intermediate/SimplifiedFramework/readability-*` and the existing public-header isolation log directories. The shader copy is a GraphicsEngine CustomBuildStep, not its empty PostBuildEvent; six generated runtime files were refreshed and hash-checked before runtime tests. Manual visual/control acceptance remains outside this follow-up. No merge, push or PR was performed.

## Camera local-pitch correction

The user reported incorrect camera rotation after the framework migration. Inspection found that ModelViewer's controls had replaced the old FreeFlyCameraController's explicit `qYaw * qPitch` composition with the generic Euler setter. That setter intentionally uses row-vector `R_yaw * R_pitch * R_roll`; with zero roll its quaternion is `qPitch * qYaw`, so pitch acts on parent X after yaw. This was a migration regression, not a renderer world-pose extraction defect, and the previous tests did not exercise turned-camera mouse pitch.

The independent testing agent reproduced the defect in Debug and Release before the fix: yaw 90° then pitch 30° left forward at `(1, 0, 0)` rather than `(0.866025, -0.5, 0)`. A sample-local upright-orientation helper now composes `qYaw * qPitch`. Camera startup and mouse look, authored camera/light target aiming, and camera-to-light aim shortcuts use the same composition. Yaw remains around actor-parent up, pitch is around the yawed camera right axis, and the existing ±89° clamp remains. This restores the earlier upright free-fly behavior; it does not introduce unrestricted roll or change generic Transform Euler semantics. Camera extraction still correctly consumes the composed world pose.

Focused regression coverage uses analytical forward/right vectors, not the new helper, to check cardinal and mixed mouse input, authored startup/no-input direction, clamping, rotated/tilted parent composition, and Shift+7/9 light alignment/spot placement. Both pre-fix Debug/Release runs failed the same turned-camera case. After correction, all four builds passed (ModelViewer and its actual-source test target in Debug/Release), both CPU control runs passed, and all four hidden runtime cases passed (threaded/sync × Debug/Release). The latter check the actual source camera/directional/spot directions against the intended focus on initial load and reload, plus existing rendering/replacement assertions. D3D queues were available in all four and reported no ERROR/CORRUPTION. No final test failures or new warning categories remain.

Independent production/test review found no actionable issues; affected project/filter paths and diff checks pass. Logs use `Intermediate/SimplifiedFramework/camera-before-*` and `camera-after-*`; `ModelViewerRuntimeTests.exe camera-controls` runs the CPU control regression without starting the graphics host. Manual mouse/visual inspection was not performed. Unrelated engine/host matrices were not repeated for this sample-local correction.
